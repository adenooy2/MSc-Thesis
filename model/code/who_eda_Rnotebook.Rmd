...
key_fact_num=key_fact %>% select(year,e_inc_num,calc_case_detect,e_mort_num) %>% gather("variable","value",2:4)
ggplot(key_fact_num,aes(x=year,y=value,group=variable,colour=variable))+geom_point()+geom_line()+theme_bw()+
...
key_fact_rate=key_fact %>% select(year,c_cdr,cfr_pct) %>% gather("variable","value",2:3)
ggplot(key_fact_rate,aes(x=year,y=value,group=variable,colour=variable))+geom_point()+geom_line()+theme_bw()+
